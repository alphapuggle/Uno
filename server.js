...
import os from "os"
import { v4 } from "uuid";
var minimumPlayers = process.env.MIN_PLAYERS || 3;
var names = [];
...
var wss = new WebSocketServer({ port: process.env.PORT || 8600 })
...
class Bot {
    constructor() {
        Bot.bots.push(this)
        this.connection = new WebSocket(`ws://localhost:${wss.options.port}`, { rejectUnauthorized: false });
        ...
        this.connection.onopen = () => {
            this.connection.onmessage = (message) => {
                let event = isJSON(message.data) && typeof (JSON.parse(message.data).event) == "string" ? JSON.parse(message.data).event.toLowerCase() : "nonjson";
                let data = isJSON(message.data) ? JSON.parse(message.data).data : message.data;
                var actions = {
                    player: () => {
...
wss.on("connection", (ws, req) => {
    ...
    ws.on("message", message => {
        let event = isJSON(message) && typeof (JSON.parse(message).event) == "string" ? JSON.parse(message).event.toLowerCase() : "nonjson";
        let data = isJSON(message) ? JSON.parse(message).data : message;
        var actions = {
            createplayer: () => {
                var duplicate = false;
